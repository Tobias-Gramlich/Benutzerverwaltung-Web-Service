...
router.post('/register', async (req, res) => {
    ...
    if (!username) return res.status(409).json({error: "Missing Username"});
    if (!password) return res.status(409).json({error: "Missing Password"});
    if (!email) return res.status(409).json({error: "Missing EMail"});
    ...
    if(await Users.findOne({where: {username: username}})) return res.status(409).json({error: "User already exists"});
    if(await Users.findOne({where: {email: email}})) return res.status(409).json({error: "There is already a user with this EMail"});
...
router.post('/activate', async (req,res) => {
    ...
    //* Check if each Component is available
    if (!username) return res.status(409).json({error: "Missing Username"});
    if (!activationcode) return res.status(409).json({error: "Missing Activation Code"});

    //* Search for User and check if it exists
    const user = await Users.findOne({ where: {username: username}});
    if (!user) return res.status(409).json({error: "User doesn't exist"});

    //* Check if ActivationCode is right and change it to 1
    if (user.activationcode !== activationcode) return res.status(409).json({error: "Wrong Activationcode"});
...
router.post('/login', async (req, res) => {
    ...
    if (user.activationcode !== 1) return res.status(409).json({error: "Account is not activated"});
...
router.post('/auth', async (req, res) => {
    ...
    //* Check if each Component is available
    if(!accessToken) return res.status(401).json({error: "Missing AccessToken"});

    //* Validate AccessToken
    const validToken = validateToken(accessToken);
    if (!validToken) return res.status(401).json({error: "User not authenticated"});
...
router.put('/change_Username', async (req, res) => {
    ...
    //* Check if each Component is available
    if(!accessToken) return res.status(401).json({error: "Missing AccessToken"});
    if(!newName) return res.status(401).json({error: "Missing New Name"});
    ...
    if(await Users.findOne({where: {username: newName}})) return res.status(409).json({error: "User already exists"});
...
router.put('/change_Password', async (req, res) => {
    ...
    //* Check if each Component is available
    if(!accessToken) return res.status(401).json({error: "Missing AccessToken"});
    if(!newPassword) return res.status(401).json({error: "Missing New Password"});
